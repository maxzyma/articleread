...
- **文字内容**：使用 `take_snapshot` 获取页面文本（标题、作者、标签等）
- **图片 URL 获取**：从 snapshot 中找到图片元素的 URL
  - 小红书图片通常在 snapshot 中显示为 `image` 元素
  - 记录所有图片的 URL（通常有多张）
- **图片文字提取**：使用 `mcp__4_5v_mcp__analyze_image` 直接分析图片 URL
  - 提示词：`"Extract all text content from this image, preserving the original structure and formatting. Be thorough and complete."`
  - 逐个处理所有图片
- **多图片处理**：小红书文章通常包含多张图片，需逐个识别并按顺序拼接

**关键要点**：
- 核心内容通常在图片中，不是 HTML 文本
- 使用图片 URL 而非截图，确保原始质量
- 图片内容要完整提取，保留原文结构
