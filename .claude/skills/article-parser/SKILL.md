---
name: article-parser
description: 从多平台提取文章内容并归档。图片/视频平台（小红书、抖音）只提取元数据，强制搜索文字版本；图文平台（微信、Twitter、Medium、知乎、博客、播客）提取全文。使用时机：用户提供任何平台文章链接或要求"解析/提取XX文章"时。
---
...
| 图片/视频平台 | 小红书、抖音、B站 | ⚠️ **只提取元数据，强制搜索文字版本** |

**⚠️ 小红书/抖音/B站处理流程（强制执行）**：

1. **步骤1：提取元数据**
   - 访问原链接，提取标题、作者、发布时间、标签等元数据
   - ⚠️ **不要提取正文内容**（图片/视频平台正文质量较低）

2. **步骤2：搜索文字版本（强制执行）**
   - 使用"标题 + 作者"搜索
   - 搜狗微信搜索 → 找到？使用微信版本 ✅
   - Google/Bing 搜索 → 找到？使用文字版本 ✅

3. **步骤3：提取全文**
   - 如果找到文字版本，使用该平台的全文提取方法
   - 元数据仍记录原始平台信息（如：小红书）

4. **步骤4：未找到文字版本**
   - ⚠️ **询问用户**："未找到文字版本，是否继续提取小红书原文？"
   - 用户确认后才提取原平台内容
...
2. **⚠️ 方法 2：验证脚本输出**（必须执行）
   - **检查空 context**：任何图片的 `context_before` 为空 → 用快照手动验证
   - **检查重复 imgIndex**：存在重复的 imgIndex → 这是异常信号，必须用快照验证
   - **检查图片内容**：下载后立即验证是否为浏览器截屏/推广内容
   - ⚠️ **空 context + 重复 imgIndex = 必须用快照验证**
