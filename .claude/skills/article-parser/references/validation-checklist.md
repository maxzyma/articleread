...
| 异常信号 | 表现 | 处理方式 |
|---------|------|---------|
| **空 context** | `context_before: ""` | 立即用快照手动验证该图片位置 |
| **imgIndex 重复** | 两张图片 imgIndex=0 | 这是异常信号，必须用快照验证 |
| **图片尺寸过大** | width > 1200 且 height > 800 | 检查是否为浏览器截屏 |
| **非文章内容** | 浏览器截屏、推广图、二维码 | 删除该图片，不插入文章 |
...
- ❌ 跳过验证 → 插入错误的图片（浏览器截屏、推广图）
- ❌ 盲目相信脚本输出 → imgIndex 重复时空 context 未处理

**正确流程**：
1. 打开微信文章
2. 运行 `extract_wechat_images.js`
3. 检查输出中是否有空 context 或重复 imgIndex
4. 如有异常，用快照手动验证图片位置
5. 下载后检查图片内容是否与上下文匹配
6. 删除不相关的图片（截屏、推广）
7. 插入正确的图片到文章
