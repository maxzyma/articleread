# 文章索引

**更新时间**: 2026-01-26
**总计**: 30 篇

---


## 2026-01

| 日期 | 标题 | 来源 | 文件路径 |
|------|------|------|----------|
| 01-26 | [Clawdbot 趋势分析 - 全面发现](general/clawdbot-trend-analysis/clawdbot-trend-analysis.md) | Google Docs | `clawdbot-trend-analysis/` |
| 01-26 | [Pencil.dev：设计与开发的真正融合](general/pencil-dev-ai-design-canvas/pencil-dev-ai-design-canvas.md) | 微信公众号 | `pencil-dev-ai-design-canvas/` |
| 01-26 | [Clawdbot一夜爆红，首个0员工公司诞生！7×24h永不下班](general/clawdbot-silicon-valley-sensation/clawdbot-silicon-valley-sensation.md) | 微信公众号 | `clawdbot-silicon-valley-sensation/` |
| 01-24 | [OpenAI 七年老将离职：大模型同质化竞赛下的行业反思](general/openai-veteran-jerry-tworek-interview/openai-veteran-jerry-tworek-interview.md) | 微信公众号 | `openai-veteran-jerry-tworek-interview/` |
| 01-23 | [Agentic Memory：让LLM像人类一样自主管理记忆](knowledge&memory/basics/agentic-memory-agemem/agentic-memory-agemem.md) | 微信公众号 | `agentic-memory-agemem/` |
| 01-23 | [AI编码实践：从Vibe Coding到SDD](general/ai-coding-vibe-to-sdd/ai-coding-vibe-to-sdd.md) | 微信公众号 | `ai-coding-vibe-to-sdd/` |
| 01-23 | [Claude Code 效率暴涨 70%？你可能正在给 Anthropic 捐款](general/claude-code-efficiency-ask-user-question-tool/claude-code-efficiency-ask-user-question-tool.md) | 微信公众号 | `claude-code-efficiency-ask-user-question-tool/` |
| 01-23 | [2026年Claude Code Skills排行榜：Top 20完整版（已去重）](general/claude-code-skills-ranking-2026/claude-code-skills-ranking-2026.md) | 微信公众号 | `claude-code-skills-ranking-2026/` |
| 01-23 | [拆解一个 3k Star 的 Skills：它到底写了什么？顺便还能学习 Manus 的上下文管理](general/planning-with-files-skill-3k-star-analysis/planning-with-files-skill-3k-star-analysis.md) | 知乎专栏 | `planning-with-files-skill-3k-star-analysis/` |
| 01-23 | [像写代码一样自动化生成短视频](general/remotion-agent-skills-video-generation/remotion-agent-skills-video-generation.md) | 微信公众号 | `remotion-agent-skills-video-generation/` |
| 01-22 | [Skills 最正确用法：将整个 GitHub 压缩成你自己的超级技能库](general/skills-github-toolbox/skills-github-toolbox.md) | Twitter/X | `skills-github-toolbox/` |
| 01-22 | [一个被忽视的 Prompt 技巧：重复提示词](general/prompt-repetition-trick/prompt-repetition-trick.md) | 微信公众号 | `prompt-repetition-trick/` |
| 01-21 | [怎么做 Long-running Agents，Cursor、Anthropic 给了两种截然不同的思路](general/long-running-agents-cursor-anthropic/long-running-agents-cursor-anthropic.md) | 微信公众号 (Founder Park) | `long-running-agents-cursor-anthropic/` |
| 01-21 | [卷死你的 Claude Code，让它彻夜干活：ralph loop 插件一手体验](general/ralph-loop-claude-code/ralph-loop-claude-code.md) | 微信公众号 | `ralph-loop-claude-code/` |
| 01-20 | [迁移到 Responses API 指南](general/openai-responses-api-migration-guide/openai-responses-api-migration-guide.md) | OpenAI 官方文档 | `openai-responses-api-migration-guide/` |
| 01-20 | [你那上万行 Agent 代码都是垃圾！Browser Use 开源 Manus 级内核](general/browser-use-agent-sdk/browser-use-agent-sdk.md) | 微信公众号 | `browser-use-agent-sdk/` |
| 01-19 | [Claude Code 之父的工作流：740 万围观背后的秘密](general/boris-claude-code-workflow/boris-claude-code-workflow.md) | 微信公众号 | `boris-claude-code-workflow/` |
| 01-18 | [Claude Cowork 发布 6 小时后：职场人案例盘点](general/claude-cowork-cases/claude-cowork-cases.md) | 小红书 (Xiaohongshu) | `claude-cowork-cases/` |
| 01-18 | [Vibe Animation：AI + 手势识别的矢量动画新流派](general/vibe-animation-tutorial/vibe-animation-tutorial.md) | 小红书 | `vibe-animation-tutorial/` |
| 01-18 | [Stitch + AI Studio：10分钟从创意到可运行的App](general/stitch-ai-studio-app-development/stitch-ai-studio-app-development.md) | 小红书 (Xiaohongshu) | `stitch-ai-studio-app-development/` |
| 01-17 | [Manus创始人：Claude是做Agent的最佳模型](general/manus-claude-best-agent-model/manus-claude-best-agent-model.md) | 抖音 (Douyin) | `manus-claude-best-agent-model/` |
| 01-13 | [Claude 降临：Claude Code 31 天实战指南](general/claude-code-31-tips/claude-code-31-tips.md) | 微信公众号 | `claude-code-31-tips/` |

## 2025-01

| 日期 | 标题 | 来源 | 文件路径 |
|------|------|------|----------|
| 01-26 | [盘点一周AI大事(1月25日)｜Agent史诗进化](general/weekly-ai-news-jan25/weekly-ai-news-jan25.md) | 小红书 | `weekly-ai-news-jan25/` |
| 01-23 | [提升 Claude 效率的优质 Skills 仓库推荐](general/claude-skills-repos-recommendation/claude-skills-repos-recommendation.md) | 微信公众号 | `claude-skills-repos-recommendation/` |
| 01-12 | [Memory vs RAG: Understanding the Difference](knowledge&memory/basics/memory-vs-rag.md) | 文档 (Supermemory) | `basics/` |
| 01-12 | [AI 记忆（Memory）的五个场景——从通用大语言模型（GPT）到图谱检索增强（GraphRAG）的卓越之路](knowledge&memory/basics/ai-memory-in-five-scenes.md) | 博客 (Cognee) | `basics/` |
| 01-12 | [停止使用检索增强生成（RAG）作为智能体记忆（Agent Memory）](knowledge&memory/solutions/stop-using-rag-for-agent-memory.md) | 博客 (Zep) | `solutions/` |
| 01-12 | [使用知识图谱（Knowledge Graph）记忆构建 AI 智能体（Agent）：Graphiti 全面指南](knowledge&memory/solutions/graphiti-knowledge-graph-memory-guide.md) | Medium | `solutions/` |
| 01-12 | [RAG vs Memory：AI 智能体的知识架构差异](knowledge&memory/comparison/rag-vs-memory-for-ai-agents.md) | 博客 (Gibson AI) | `comparison/` |
| 01-12 | [为什么我们构建 Responses API](general/openai-responses-api/openai-responses-api.md) | 博客 (OpenAI) | `openai-responses-api/` |

---

## 按主题统计

- **General (通用)**: 24 篇
- **Knowledge & Memory (知识与记忆)**: 6 篇

---

**说明**：此索引由脚本自动生成。更新命令：

```bash
python3 scripts/update_article_index.py
```
